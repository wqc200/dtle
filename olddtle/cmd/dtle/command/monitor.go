...
func (m *monitor) update(update *evalState) {
		...
		if existing, ok := existing.allocs[allocID]; !ok {
			...
			case alloc.index < update.index:
				...
				m.ui.Output(fmt.Sprintf(
					"Allocation %q modified: node %q, task %q",
					limit(alloc.id, m.length), limit(alloc.node, m.length), alloc.task))

			case alloc.desired == models.AllocDesiredStatusRun:
				// New allocation with desired status running
				m.ui.Output(fmt.Sprintf(
					"Allocation %q created: node %q, task %q",
					limit(alloc.id, m.length), limit(alloc.node, m.length), alloc.task))
			}
		} else {
			switch {
	...
	if existing.status != "" &&
		update.status != models.AllocClientStatusPending &&
	...
	for {
		...
		if err != nil {
			...
			if len(evals) > 1 {
				...
				out[0] = "ID|Type|Triggered By|Status"
		...
		switch eval.Status {
		case models.EvalStatusComplete, models.EvalStatusFailed, models.EvalStatusCancelled:
			if len(eval.FailedTGAllocs) == 0 {
				...
				// Print the failures per task
				for t, metrics := range eval.FailedTGAllocs {
					noun := "allocation"
					if metrics.CoalescedFailures > 0 {
						noun += "s"
					}
					m.ui.Output(fmt.Sprintf("Task %q (failed to place %d %s):", t, metrics.CoalescedFailures+1, noun))
