...
func (b *EvalBroker) Enqueue(eval *models.Evaluation) {
	b.l.Lock()
	defer b.l.Unlock()
...
func (b *EvalBroker) EnqueueAll(evals map[*models.Evaluation]string) {
...
func (b *EvalBroker) processEnqueue(eval *models.Evaluation, token string) {
...
func (b *EvalBroker) enqueueWaiting(eval *models.Evaluation) {
	b.l.Lock()
	defer b.l.Unlock()
...
func (b *EvalBroker) enqueueLocked(eval *models.Evaluation, queue string) {
	...
	if !ok {
		pending = make([]*models.Evaluation, 0, 16)
...
func (b *EvalBroker) Dequeue(schedulers []string, timeout time.Duration) (*models.Evaluation, string, error) {
...
func (b *EvalBroker) scanForSchedulers(schedulers []string) (*models.Evaluation, string, error) {
	b.l.Lock()
	defer b.l.Unlock()
...
func (b *EvalBroker) dequeueForSched(sched string) (*models.Evaluation, string, error) {
	// Get the pending queue
...
func (b *EvalBroker) EmitStats(period time.Duration, stopCh chan struct{}) {
	for {
		...
		case <-time.After(period):
			stats := b.Stats()
			metrics.SetGauge([]string{"server", "broker", "total_ready"}, float32(stats.TotalReady))
			metrics.SetGauge([]string{"server", "broker", "total_unacked"}, float32(stats.TotalUnacked))
			metrics.SetGauge([]string{"server", "broker", "total_blocked"}, float32(stats.TotalBlocked))
			metrics.SetGauge([]string{"server", "broker", "total_waiting"}, float32(stats.TotalWaiting))
			for sched, schedStats := range stats.ByScheduler {
				metrics.SetGauge([]string{"server", "broker", sched, "ready"}, float32(schedStats.Ready))
				metrics.SetGauge([]string{"server", "broker", sched, "unacked"}, float32(schedStats.Unacked))
			}
...
func (p PendingEvaluations) Peek() *models.Evaluation {
	n := len(p)
	if n == 0 {
		return nil
	}
