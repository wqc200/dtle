...
import (
	...
	"github.com/actiontech/dtle/olddtle/internalinternal/client/driver"
	"github.com/actiontech/dtle/olddtle/internalinternal/models"
	"github.com/actiontech/dtle/olddtle/internalinternal/server/scheduler"
				...
				for _, t := range jobCopy.Tasks {
					if connCfg, ok := t.Config["ConnectionConfig"]; ok {
						if connCfgMap, ok := connCfg.(map[string]interface{}); ok {
							//getStar := func() string { return "*" }
							connCfgMap["Password"] = MaskedPassword
						}
					}
				}
