...
func (p *pendingPlan) Wait() (*models.PlanResult, error) {
	err := <-p.errCh
	return p.result, err
}

// respond is used to set the response and error for the future
func (p *pendingPlan) respond(result *models.PlanResult, err error) {
	p.result = result
	p.errCh <- err
}
...
func (q *PlanQueue) Enqueue(plan *models.Plan) (PlanFuture, error) {
	q.l.Lock()
	defer q.l.Unlock()
