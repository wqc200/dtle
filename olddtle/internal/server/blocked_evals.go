...
func (b *BlockedEvals) Block(eval *models.Evaluation) {
	b.processBlock(eval, "")
}
...
// evaluation's token.
func (b *BlockedEvals) Reblock(eval *models.Evaluation, token string) {
...
// may be outstanding.
func (b *BlockedEvals) processBlock(eval *models.Evaluation, token string) {
	b.l.Lock()
	defer b.l.Unlock()
	...
	if b.missedUnblock(eval) {
		...
		// still outstanding.
		b.evalBroker.EnqueueAll(map[*models.Evaluation]string{eval: token})
		return
	}
...
// the lock held.
func (b *BlockedEvals) missedUnblock(eval *models.Evaluation) bool {
	var max uint64 = 0
...
func (b *BlockedEvals) unblock(computedClass string, index uint64) {
	...
	unblocked := make(map[*models.Evaluation]string, lib.MaxInt(numEscaped, 4))
	if numEscaped != 0 {
...
func (b *BlockedEvals) UnblockFailed() {
	...
	unblocked := make(map[*models.Evaluation]string, 4)
	for id, wrapped := range b.captured {
		if wrapped.eval.TriggeredBy == models.EvalTriggerMaxPlans {
			unblocked[wrapped.eval] = wrapped.token
			delete(b.captured, id)
			delete(b.jobs, wrapped.eval.JobID)
		}
...
// passed timeout.
func (b *BlockedEvals) GetDuplicates(timeout time.Duration) []*models.Evaluation {
...
func (b *BlockedEvals) EmitStats(period time.Duration, stopCh chan struct{}) {
	for {
		...
		case <-time.After(period):
			stats := b.Stats()
			metrics.SetGauge([]string{"server", "blocked_evals", "total_blocked"}, float32(stats.TotalBlocked))
			metrics.SetGauge([]string{"server", "blocked_evals", "total_escaped"}, float32(stats.TotalEscaped))
		case <-stopCh:
			return
		}
	}
