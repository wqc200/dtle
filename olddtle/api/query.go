...
type QueryOptions struct {
	...
	// AllowStale allows any Udup server (non-leader) to service
...
type Config struct {
	// Address is the address of the Udup agent
	Address string
...
func (c *Config) CopyConfig(address string) *Config {
	scheme := "http"
	config := &Config{
...
func DefaultConfig() *Config {
	config := &Config{
		Address:    "http://127.0.0.1:8190",
		HttpClient: cleanhttp.DefaultClient(),
	}

	if addr := os.Getenv("UDUP_ADDR"); addr != "" {
		config.Address = addr
	}
	if auth := os.Getenv("UDUP_HTTP_AUTH"); auth != "" {
...
// Client provides a client to the Udup API
type Client struct {
	config Config
}
...
// and serialize/deserialized using the standard Udup conventions.
...
func parseQueryMeta(resp *http.Response, q *QueryMeta) error {
	...
	// Parse the X-Udup-Index
	index, err := strconv.ParseUint(header.Get("X-Udup-Index"), 10, 64)
	if err != nil {
		return fmt.Errorf("Failed to parse X-Udup-Index: %v", err)
	}
	q.LastIndex = index

	// Parse the X-Udup-LastContact
	last, err := strconv.ParseUint(header.Get("X-Udup-LastContact"), 10, 64)
	if err != nil {
		return fmt.Errorf("Failed to parse X-Udup-LastContact: %v", err)
	}
	q.LastContact = time.Duration(last) * time.Millisecond

	// Parse the X-Udup-KnownLeader
	switch header.Get("X-Udup-KnownLeader") {
	case "true":
		q.KnownLeader = true
	default:
		q.KnownLeader = false
