...
def build(version=None,
    ...
    for target, path in targets.items():
        ...
        if "1.4" in get_go_version():
            if static:
                build_command += "-ldflags=\"-s -X main.Version {} -X main.GitBranch {} -X main.GitCommit {}\" ".format(version,
                                                                                                                  get_current_branch(),
                                                                                                                  get_current_commit())
            else:
                build_command += "-ldflags=\"-X main.Version {} -X main.GitBranch {} -X main.GitCommit {}\" ".format(version,
                                                                                                               get_current_branch(),
                                                                                                               get_current_commit())
